...
package esb_webservice.routingsplit_0_1;
...
/**
 * Job: RoutingSplit Purpose: <br>
 * Description: Route a message switch different situation <br>
 * 
...
public class RoutingSplit extends org.apache.camel.builder.RouteBuilder implements TalendESBRoute {
	...
	public void setArguments(Object arguments) {
		this.inOSGi = true;
		RoutingSplit.arguments = arguments;
	...
	public void doConfig() throws java.lang.Exception {
		...
		from("direct:biling").routeId("RoutingSplit_cDirect_4").process(new org.apache.camel.Processor() {
			public void process(org.apache.camel.Exchange exchange) throws Exception {
				System.out
						.println("Message received on endpoint biling: " + exchange.getIn().getHeader("CamelFileName"));
			}

		}).id("RoutingSplit_cProcessor_1");
		from("direct:discount").routeId("RoutingSplit_cDirect_5").process(new org.apache.camel.Processor() {
			public void process(org.apache.camel.Exchange exchange) throws Exception {
				System.out.println(
						"Message received on endpoint discount: " + exchange.getIn().getHeader("CamelFileName"));
			}

		}).id("RoutingSplit_cProcessor_2");
		from("direct:receipt").routeId("RoutingSplit_cDirect_6").process(new org.apache.camel.Processor() {
			public void process(org.apache.camel.Exchange exchange) throws Exception {
				System.out.println(
						"Message received on endpoint receipt: " + exchange.getIn().getHeader("CamelFileName"));
			}

		}).id("RoutingSplit_cProcessor_3");
		from("file://" + context.dir_path + "RoutingSplit/in" + "?noop=true" + "&autoCreate=true" + "&flatten=false"
				+ "&charset=UTF-8" + "&bufferSize=128").routeId("RoutingSplit_cFile_1")
						.to("flatpack:foo?delimiter=;&ignoreFirstRecord=true&splitRows=true")
						.id("RoutingSplit_cMessagingEndpoint_3");
		from("flatpack:foo?delimiter=;&ignoreFirstRecord=true&splitRows=true")
				.routeId("RoutingSplit_cMessagingEndpoint_5").setHeader("mySlip")
				.constant("direct:biling,direct:receipt,direct:discount").id("RoutingSplit_cSetHeader_1")
				.routingSlip(header("mySlip"), ",").id("RoutingSplit_cRoutingSlip_1");
	}
			...
			protected CamelContext createContext() {
				final org.apache.camel.impl.DefaultCamelContext camelContext = new org.apache.camel.spring.SpringCamelContext(
						new org.springframework.context.support.ClassPathXmlApplicationContext(
								"META-INF/spring/routingsplit.xml"));
				camelContext.setName("RoutingSplit");
				// add notifier
				...
				runStat.setAllPID(pid, pid, pid, "RoutingSplit");
				try {
				...
				targetNodeToConnectionMap.put("RoutingSplit_cProcessor_1", "route2");
				targetNodeToConnectionMap.put("RoutingSplit_cProcessor_2", "route8");
				targetNodeToConnectionMap.put("RoutingSplit_cProcessor_3", "route9");
				targetNodeToConnectionMap.put("RoutingSplit_cMessagingEndpoint_3", "route1");
				targetNodeToConnectionMap.put("RoutingSplit_cSetHeader_1", "route10");
				targetNodeToConnectionMap.put("RoutingSplit_cRoutingSlip_1", "route3");
	...
	public static void main(String[] args) {
		int exitCode = new RoutingSplit().runJobInTOS(args);
		if (exitCode != 0) {
	...
	private void readContextValues(String contextName) {
		...
		try {
			java.io.InputStream inContext = RoutingSplit.class.getClassLoader()
					.getResourceAsStream("esb_webservice/routingsplit_0_1/contexts/" + contextName + ".properties");
